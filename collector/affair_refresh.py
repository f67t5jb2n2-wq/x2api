...
def refresh_playback_urls(conn, limit: int, refresh_window_minutes: int, critical_window_minutes: int) -> dict[str, int]:
    ...
    queries = [
        (
            ...
            LIMIT %s
            """,
            (AFFAIR_SOURCE, critical_window_minutes, limit),
        ),
        (
            """
            SELECT i.*
            ...
            LIMIT %s
            """,
            (AFFAIR_SOURCE, refresh_window_minutes, limit),
        ),
    ]
    ...
    for sql, params in queries:
        ...
        for row in rows:
            ...
            video_id = metadata.get("affair_video_id")
            try:
                if not source_url:
                    detail_id = metadata.get("affair_detail_id") or str(row["guid"]).replace(f"{AFFAIR_SOURCE}:", "", 1).split(":", 1)[0]
                    source_url = f"{(metadata.get('target_value') or AFFAIR_DEFAULT_BASE_URL).rstrip('/')}/archives/{detail_id}/"
                if not source_url:
                ...
                detail = parse_detail_page(
                    ...
                    {
                        "guid": row["guid"],
                        "detail_id": metadata.get("affair_detail_id"),
                        "url": source_url,
                        "title": row.get("title"),
