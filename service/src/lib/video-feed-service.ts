...
export async function recordVideoImpressions(input: {
  clientId: string;
  itemIds: string[];
  metadata?: Record<string, unknown>;
}) {
  const itemIds = [...new Set(input.itemIds.map((itemId) => itemId.trim()).filter(Boolean))];
  if (itemIds.length === 0) {
    return { recorded: 0 };
  }
  ...
  const rows = asRows<{ id: string }>(await sql`
    ...
    inserted_events AS (
      ...
      SELECT ${input.clientId}, selected_items.id, 'impression', NULL, ${JSON.stringify(metadata)}::jsonb
      FROM selected_items
      RETURNING item_id
    ),
    stat_values AS (
